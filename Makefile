# This Makefile is for the CSVDB extension to perl.
#
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/CSVDB.pm $(INST_MAN3DIR)/CSVDB.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Access a CSV file like a database'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: CSVDB' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Access a CSV file like a database",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "T. Alex Beamish <talexb@gmail.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "CSVDB",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="CSVDB" VERSION="0.01">' > CSVDB.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Access a CSV file like a database</ABSTRACT>' >> CSVDB.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>T. Alex Beamish &lt;talexb@gmail.com&gt;</AUTHOR>' >> CSVDB.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> CSVDB.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> CSVDB.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.30" />' >> CSVDB.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> CSVDB.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> CSVDB.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> CSVDB.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/CSVDB.pm' 'blib/lib/CSVDB.pm' 
